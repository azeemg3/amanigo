...
@section('content')
                            ...
                            <div class="row invoice-info">
                                ...
                                <div class="col-sm-4 invoice-col">
                                    <address>
                                        ...
                                        <strong>Taken Over By: </strong><span class="float-right">{{ $result[0]->taken_ob }}</span><br>
                                        ...
                                        <span class="ledger">
                                            ...
                                            <button class="btn-xs btn btn-info btn-flat" onclick="get_lead_details()">Create Ledger</button></span>
                                            @endif
                                        @if($result[0]->ledger==1)
                                            {{--Ledger Request Sent--}}
                                            @endif
                                        @if($result[0]->status==1)
                                            <button class="btn-xs btn btn-danger btn-flat" data-toggle="modal" data-target="#close-lead-modal">Closed Lead</button>
                                            <button class="btn-xs btn btn-warning btn-flat" data-toggle="modal" data-target="#transfer-lead-modal">Transfer Lead</button>
                                        @endif
                                        @if($result[0]->status==4)
                                            <button onclick="reopen_lead({{ $result[0]->id }})" class="btn-xs btn btn-primary btn-flat">Reopen Lead</button>
                                            @endif
                                    </address>
                                </div>
                                <!-- /.col -->
    ...
    @include('Lms.close-lead')
    @include('Lms.transfer-lead-modal')
    @include('Lms.js_func')
